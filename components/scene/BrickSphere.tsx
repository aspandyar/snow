...
export default function BrickSphere() {
  ...
  // аналитически. Луч по самим инстансам перебирал бы все 828 копий по
  ...
  //
  // Направление входа берётся из конфига — от ПОСЛЕДНЕЙ точки траектории
  // к центру сферы, то есть оттуда, где камера в итоге останавливается.
  // Не от её настоящего положения каждый кадр: проём обязан открываться в
  // ОДНОМ месте, иначе дыра поедет по кладке вслед за камерой.
  //
  // Раньше здесь стояла ПРЕДПОСЛЕДНЯЯ точка — тогда последняя лежала
  // внутри сферы и годились обе, они на одном луче. Когда путь укоротили
  // и последняя точка переехала наружу, предпоследняя оказалась на 45°
  // в стороне: проём открывался мимо камеры, и та упиралась в кладку.
  const проём = useMemo(() => {
    const последняя = траектория.позиции[траектория.позиции.length - 1]
    const вход = new Vector3(
      -последняя[0],
      сцена.высотаПарения - последняя[1],
      -последняя[2],
    ).normalize()
    ...
    // не в массиве объектов Vector3: восемь сотен объектов читаются с
  ...
  const порог = useMemo(
    () =>
      конецСцены(прокрутка.разгораниеВЭкранах, прокрутка.нахлёстВЭкранах, прокрутка.экранов),
    [],
  )
