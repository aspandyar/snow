...
export default function BrickAxes() {
  const оси = useMemo(() => {
    ...
    for (let i = 0; i < кирпичи.length; i += отладка.шагВыборки) {
      ...
      // Центр кирпича лежит на радиусе минус половина толщины, то есть
      // внутри оболочки. Наружу из центра торчит только синяя ось нормали —
      // она и так направлена наружу, а красная (вдоль кольца) и зелёная
      // (вдоль меридиана) лежат по касательной и целиком тонут в толще
      // соседних кирпичей. Поэтому началом берём точку на внешней грани:
      // центр плюс нормаль на половину толщины.
      const начало = к.позиция
        .clone()
        .addScaledVector(к.нормаль, сфера.толщина / 2)

      for (const [направление, цвет] of направления) {
        результат.push({
          начало: начало.clone(),
          конец: начало.clone().addScaledVector(направление, отладка.длинаОси),
          цвет,
        })
      }
    }
