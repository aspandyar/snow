...
- Имена React-компонентов — тоже латиницей, и по такой же принудительной
  причине. Правило `react-hooks/rules-of-hooks` отличает компонент от
  обычной функции проверкой `/^[A-Z]/`, зашитой в исходник плагина без
  единой настройки: кириллическая заглавная не подходит под неё никогда.
  С русскими именами линтер считал все компоненты сцены обычными
  функциями и выдавал два десятка ложных ошибок, в которых настоящая —
  хук, вызванный в условии или цикле, — потерялась бы бесследно.
  Идентификаторы внутри компонентов, свойства, типы и всё остальное
  остаются русскими: латиница нужна ровно там, где её требует чужой
  инструмент.
